...
)
{
    ...
    __try {
        ...
        if (!NT_SUCCESS(status)) { // завершение работы при неудаче
            __leave;
        }


        //также вместо nameInfo->Extension можно извлечь расширение из имени файла, разделив по точке
    ...
    __finally {
        // освобождение памяти аналогично https://github.com/microsoft/Windows-driver-samples/blob/main/filesys/miniFilter/swapBuffers/swapBuffers.c
        if (nameInfo != NULL) {
            FltReleaseFileNameInformation(nameInfo); // !!! ОБЯЗАТЕЛЬНО ОЧИСТКА ПАМЯТИ !!!
            nameInfo = NULL;
        }
    }
